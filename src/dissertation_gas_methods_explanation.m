%% Breakdown into pieces of the Gas Methods function
% First for the function to run, we should load appropriate input
...
% not be used... Still I will  not fix it unless I have to.
        ...
        [savestructure.gas(j).bestmatch, savestructure.gas(j).bestmatchbyindex] = genbestmmatrix(savestructure.gas(j).nodes, savestructure.train.gas(j).inputs.input, arq_connect.layertype, arq_connect.q); %assuming the best matching node always comes from initial dataset!
%end