function PCA_hBLabs(featureAllMean,labels,fig_id)
...
elseif nargin==3
    ...
    circle_vis=0;
elseif nargin==2
    labels=ones(1,size(featureAllMean,1));
elseif nargin==1 
    labels=ones(1,size(featureAllMean,1));
    fig_id=1;
end;
...
subplot(2,2,2);
   ...
   % else
    ...
    for i=1:size(X,1)
          plot(X(i,1),X(i,2),'--rs','MarkerSize',Msize(i),'MarkerEdgeColor',labels2(i,:),'MarkerFaceColor',labels2(i,:));%,'Marker',labels_cells(i).Marker,'MarkerEdgeColor',labels_cells(i).colorVector,'MarkerSize',labels_cells(i).MarkerSize,'MarkerFaceColor',labels_cells(i).MarkerFaceColor); 
          if circle_vis
              X(i,1);
              SD(i,1);
