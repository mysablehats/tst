...
subplot(2,2,2);
   ...
   % else
    ...
    hold on;
    for i=1:size(X,1)
          plot(X(i,1),X(i,2),'--rs','MarkerSize',5,'MarkerEdgeColor',labels{i},'MarkerFaceColor',labels{i});%,'Marker',labels_cells(i).Marker,'MarkerEdgeColor',labels_cells(i).colorVector,'MarkerSize',labels_cells(i).MarkerSize,'MarkerFaceColor',labels_cells(i).MarkerFaceColor); 
          if circle_vis
              X(i,1);
              SD(i,1);
